...
use ibc_proto::ibc::core::{
    ...
    client::v1::{
        ...
        ConsensusStateWithHeight, Height, IdentifiedClientState, MsgCreateClient,
...
pub struct IbcGrpcService<Store: IbcStore, Ctx: Ics26Context> {
    store: Arc<Store>,
    addr: SocketAddr,
    ctx: Arc<RwLock<Ctx>>,
}

impl<Store, Ctx> IbcGrpcService<Store, Ctx>
where
    Store: IbcStore + 'static,
    Ctx: Ics26Context + Sync + Send + 'static,
{
    pub fn new(store: Store, addr: String, ctx: Ctx) -> Self {
        IbcGrpcService {
            store: Arc::new(store),
